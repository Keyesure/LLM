...
class CalculatorTool(BaseTool):
    name = "CalculatorTool"
    description = """
    当需要回答关于数学问题或是需要运行Python程序的问题时，你可以使用这个工具，输入按照以下格式：已知：xxx，求xxx
    """
...
tools = [
    ...
    Tool(
        name="计算器",
        description="当需要回答关于数学问题或是需要运行Python程序的问题时，你可以使用这个工具，输入按照以下格式：已知：xxx，求xxx",
        func=cal.run,
    ),
    Tool(
        name="维基百科",
    ...
    Tool(
        name="数据库",
        func=sql_agent.run,
        description="这个工具用来执行数据库查询，适合回答与下列几个表有关的问题"
